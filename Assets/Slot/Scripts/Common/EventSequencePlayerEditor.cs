...
using Spine.Unity;
using DG.Tweening; // Added for Inspector visibility of EaseType
...
public class EventSequencePlayerEditor : Editor
{
    ...
    void SetupLists()
    {
        ...
        for (int i = 0; i < sequencesProp.arraySize; i++)
        {
            ...
            list.elementHeightCallback = (int index) =>
            {
                ...
                if (!isExpanded.boolValue) return line + (space * 2);
                ...
                switch (eventType)
                {
                    ...
                    // NEW: Transform Modifier Height
                    case EventType.ModifyTransform:
                        height += (line + space) * 2; // Duration, Ease

                        height += line + space; // Modify Pos Bool
                        if (element.FindPropertyRelative("modifyPosition").boolValue) height += line + space;

                        height += line + space; // Modify Rot Bool
                        if (element.FindPropertyRelative("modifyRotation").boolValue) height += line + space;

                        height += line + space; // Modify Scale Bool
                        if (element.FindPropertyRelative("modifyScale").boolValue) height += line + space;

                        height += line + space; // WaitForCompletion
                        break;
                }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("▶ Play", GUILayout.Height(30))) player.Play();
        GUI.backgroundColor = Color.red;
        if (GUILayout.Button("⏹ Stop", GUILayout.Height(30))) player.Stop();
    ...
    void DrawSequences()
    {
        for (int i = 0; i < sequencesProp.arraySize; i++)
        {
            ...
            if (i < eventLists.Count) eventLists[i].DoLayoutList();
    ...
    void DrawEvent(Rect rect, SerializedProperty element)
    {
        SerializedProperty isExpanded = element.FindPropertyRelative("isExpanded");
        ...
        if (iconContent != null) EditorGUI.LabelField(iconRect, iconContent);
        ...
        if (!isExpanded.boolValue) return;
        ...
        if (eventType != EventType.UnityEvent && eventType != EventType.PlaySequence)
        {
        ...
        switch (eventType)
        {
            case EventType.UnityEvent:
                ...
                EditorGUI.PropertyField(new Rect(rect.x + 8, y, rect.width - 16, unityHeight), unityEvent, true);
                y += unityHeight + space;
                break;
            ...
            case EventType.EnableObject:
                SerializedProperty playOnEnable = element.FindPropertyRelative("playOnEnable");
                EditorGUI.PropertyField(new Rect(rect.x + 8, y, rect.width - 16, line), playOnEnable);
                y += line + space;
                break;
            ...
            case EventType.PlaySequence:
                ...
                EditorGUI.PropertyField(new Rect(rect.x + 8, y, rect.width - 16, line), sequencePlayerTarget, new GUIContent("Target Player"));
                y += line + space;
                EditorGUI.PropertyField(new Rect(rect.x + 8, y, rect.width - 16, line), targetSequenceId, new GUIContent("Sequence ID"));
                y += line + space;
                EditorGUI.PropertyField(new Rect(rect.x + 8, y, rect.width - 16, line), waitForCompletion);
                y += line + space;
                break;

            // NEW: Transform Modifier Drawing logic
            case EventType.ModifyTransform:
    ...
    void DrawSpineDropdown(Rect rect, ref float y, float line, SerializedProperty targetProp, SerializedProperty animProp)
    {
        ...
        if (skeleton == null) skeleton = target.GetComponent<SkeletonGraphic>();
        ...
        foreach (var anim in skeleton.Skeleton.Data.Animations) names.Add(anim.Name);
        ...
        int newIndex = EditorGUI.Popup(new Rect(rect.x + 8, y, rect.width - 16, line), "Animation", currentIndex, names.ToArray());
        animProp.stringValue = names[newIndex];
        y += line + 6;
    }

    GUIContent GetIconForEvent(EventType type)
    {
        switch (type)
        {
            case EventType.UnityEvent: return EditorGUIUtility.IconContent("cs Script Icon");
            case EventType.PlayAnimation: return EditorGUIUtility.IconContent("AnimationClip Icon");
            case EventType.PlaySpineAnimation: return EditorGUIUtility.IconContent("Avatar Icon");

            // FIXED: Using universally available icons for Enable/Disable
            case EventType.EnableObject: return EditorGUIUtility.IconContent("Toolbar Plus");
            case EventType.DisableObject: return EditorGUIUtility.IconContent("Toolbar Minus");

            case EventType.PlayAudio: return EditorGUIUtility.IconContent("AudioSource Icon");
            case EventType.PlaySequence: return EditorGUIUtility.IconContent("d_PlayButton");
            case EventType.ModifyTransform: return EditorGUIUtility.IconContent("Transform Icon");
            default: return null;
        }
    }
    ...
    void DrawBackground(Rect rect, EventType type)
    {
        ...
        switch (type)
        {
            case EventType.PlayAnimation: GUI.backgroundColor = new Color(0.7f, 0.9f, 1f); break;
            case EventType.PlaySpineAnimation: GUI.backgroundColor = new Color(0.8f, 0.7f, 1f); break;
            case EventType.EnableObject: GUI.backgroundColor = new Color(0.8f, 1f, 0.8f); break;
            case EventType.DisableObject: GUI.backgroundColor = new Color(1f, 0.8f, 0.8f); break;
            case EventType.PlayAudio: GUI.backgroundColor = new Color(1f, 0.9f, 0.6f); break;
            case EventType.PlaySequence: GUI.backgroundColor = new Color(1f, 0.7f, 0.4f); break;
            case EventType.ModifyTransform: GUI.backgroundColor = new Color(0.6f, 0.9f, 0.9f); break; // Cyan tint
            default: GUI.backgroundColor = new Color(0.9f, 0.9f, 0.9f); break;
        }
